...
def descriptor_resnet_symbol(num_res):
    ...
    running_mean = {}
    running_var = {}
    gamma['batchnorm0'] = mx.sym.Variable('batchnorm0'+'_gamma')
    beta['batchnorm0'] = mx.sym.Variable('batchnorm0'+'_beta')
    running_mean['batchnorm0'] = mx.sym.Variable('batchnorm0'+'_running_mean')
    running_var['batchnorm0'] = mx.sym.Variable('batchnorm0'+'_running_var')
    conv0 = mx.sym.Convolution(name='conv0', data=data, num_filter=64, pad=(0,0), kernel=(7,7), stride=(2,2), no_bias=True, workspace=1024, weight=weight['conv0'])
    bn0 = mx.sym.BatchNorm(name='bn0', data=conv0, momentum=0.9, gamma=gamma['batchnorm0'], beta=beta['batchnorm0'], moving_mean=running_mean['batchnorm0'], moving_var=running_var['batchnorm0'])
    ...
    out = resnet_symbol(data, weight_var, beta_var, gamma_var, running_mean_var, running_var_var, 0)
    for i in range(num_res-1):
        weight_var = {}
        beta_var = {}
        gamma_var = {} 
        ...
        name='batchnorm0'
        data = mx.sym.BatchNorm(data=data, momentum=0.9, name=name, gamma=gamma_var[name], beta=beta_var[name], moving_mean=running_mean_var[name], moving_var=running_var_var[name])
        out = mx.sym.Group([out, resnet_symbol(data, weight_var, beta_var, gamma_var, running_mean_var, running_var_var, 1)])
    return out

def resnet_symbol(data, weight, beta, gamma, running_mean, running_var, i):
    def conv(name, data, num_filter, pad, kernel, stride, no_bias, workspace):
        return mx.sym.Convolution(name=name, data=data, weight=weight[name], num_filter=num_filter, pad=pad, kernel=kernel, stride=stride, no_bias=no_bias, workspace=workspace)
    def bn(name, data, momentum):
    ...
    conv1_1 = conv(name='conv'+str(i), data=data , num_filter=64, pad=(0,0), kernel=(3,3), stride=(2,2), no_bias=True, workspace=1024)
    bn1_1 = bn(name='batchnorm'+str(i), data=conv1_1, momentum=0.9)
    relu1_1 = mx.symbol.Activation(name='relu1_1', data=bn1_1 , act_type='relu')
    conv1_2 = conv(name='conv'+str(i+1), data=relu1_1 , num_filter=64, pad=(0,0), kernel=(3,3), stride=(2,2), no_bias=True, workspace=1024)
    bn1_2 = bn(name='batchnorm'+str(i+1), data=conv1_2, momentum=0.9)
    conv2_1 = conv(name='conv'+str(i+2), data=bn1_2 , num_filter=64, pad=(0,0), kernel=(3,3), stride=(2,2), no_bias=True, workspace=1024)
    ...
    conv2_2 = conv(name='conv'+str(i+3), data=relu2_1 , num_filter=64, pad=(0,0), kernel=(3,3), stride=(2,2), no_bias=True, workspace=1024)
